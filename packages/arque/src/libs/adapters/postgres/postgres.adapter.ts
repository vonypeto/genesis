...
export class PostgresStoreAdapter implements StoreAdapter {
  ...
  constructor(opts?: Partial<Options>) {
    this.opts = {
      connectionString:
        ...
        'postgresql://postgres:postgres@localhost:5432/arque',
  ...
  private serialize(value: unknown) {
    return match(value)
      ...
      .otherwise((value) => this.joser.serialize(value as any));
  }
  ...
  private deserialize(value: unknown) {
    ...
    return this.joser.deserialize(value as any);
  }
  ...
  async saveEvents(params: {
    ...
    try {
      ...
      const aggRes = await client.query(
        `SELECT final FROM "${this.schema}".aggregates WHERE id = $1`,
        [params.aggregate.id]
      );
          ...
          for (let i = 0; i < params.events.length; i++) {
            ...
            await client.query(
              ...
              [
                ...
                JSON.stringify(
                  this.serialize({ ...event.meta, ...params.meta })
                ),
                params.timestamp,
              ]
            );
