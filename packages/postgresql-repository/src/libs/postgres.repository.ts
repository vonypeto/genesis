...
export class PostgresRepository<T extends { id?: string }>
    ...
    private mapTypeToPostgres(type: any): string {
        ...
        if (type === Number || (type.type && type.type === Number)) return 'NUMERIC';
        if (type === Boolean || (type.type && type.type === Boolean)) return 'BOOLEAN';
    ...
    async update(
        ...
        if (res.rowCount === 0 && options?.upsert) {
            return this.create({ ...data, id: id as string } as Partial<T>);
        }
    ...
    async updateOne(
        ...
        return this.update(found.id!, dataToUpdate, options);
    }
    ...
    async updateMany(
        ...
        for (const item of items) {
            await this.update(item.id!, update as any); // reusing logic
            modifiedCount++;
        }
    ...
    async countAll(): Promise<number> {
        const res = await this.pool.query(`SELECT COUNT(*) FROM "${this.tableName}";`);
    ...
    async aggregate(pipeline: any[]): Promise<any[]> {
        throw new Error('Aggregation not fully supported in PostgresRepository yet');
    }
    ...
    private buildWhereClause(filter: FilterQuery<T>): { whereClause: string, values: any[] } {
